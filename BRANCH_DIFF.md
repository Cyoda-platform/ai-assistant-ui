...
> **Контекст:**
> - `develop` — **старый проект, рабочий UI** (оригинальная базовая ветка)
> - `cyoda-cloud-workbench-ui` — **новый UI** (ребрендинг + новая главная страница, сделан поверх develop)
>
> **Соглашение по знакам в диффах** (`git diff cyoda-cloud-workbench-ui develop`):
> - строки с `-` — код в `cyoda-cloud-workbench-ui` (новый UI, что было убрано по сравнению с develop)
> - строки с `+` — код в `develop` (старый UI, что было в оригинале)

---

## Резюме

Ветка `cyoda-cloud-workbench-ui` — это **новый UI поверх `develop`**: светлая тема заменена на тёмную (dark mode), брендинг «CYODA AI» / «CYODA AI Studio» вместо старого, добавлена маркетинговая главная страница (FintechHomeView), улучшен LoginButton (добавлена проверка авторизации и `returnTo`), добавлен `deepMerge` для переводов.

**Бэкенд при этом не менялся** — изменился только UI, главная страница и названия в интерфейсе.

**Однако при переходе на тёмную тему были допущены ошибки**, из-за которых чат перестал работать:
- `ChatBotView` оставили со старым светлым фоном (`bg-slate-50`) при тёмной теме
- `StreamingMessage` использует `prose` без `prose-invert` на тёмном фоне
- Также обнаружена несвязанная проблема: бэкенд изменил поле ответа с `technical_id` на `chat_id`, что сломало создание чата в обеих ветках

**Дополнение:** `App.tsx` тоже изменился (не попал в первоначальный список — diff stat был обрезан): изменена логика редиректа после логина, добавлен автоматический редирект авторизованных пользователей с `/` на `/home`, добавлен CSS-класс `marketing-page` для лендинга.
...
### Что нового в cyoda-cloud-workbench-ui (относительно старого develop)

Новый UI добавил следующие **intentional** изменения:
- Тёмная тема принудительно (`theme-dark` везде, `setTheme` игнорирует аргумент)
- Ребрендинг: `'Cyoda Cloud'` → `'CYODA AI Assistant'`, бейдж `'BETA'` → `'ALPHA'`
- Новая маркетинговая главная страница (`FintechHomeView` с GA, FAQ, своим auth-flow)
- Улучшенный `LoginButton`: добавлена проверка `isAuthenticated`, `returnTo`, обработка ошибок
- `deepMergeTranslations` в i18n — серверные переводы теперь мёржатся с локальными
- Lazy loading для всех вьюх через `React.lazy()` + `Suspense`
- Редирект авторизованных пользователей с `/` на `/home` (App.tsx)
- Шрифты Inter / JetBrains Mono в Tailwind config
- Переориентация примеров запросов на финтех/workflow-специфику Cyoda
- Год копирайта: `2025` → `2026`

### Баги, допущенные при переходе на тёмную тему (исправлены в cyoda-cloud-workbench-ui-2)

1. **ChatBotView — светлый фон в тёмном приложении** ✅ исправлено:
   - Было: `bg-slate-50 text-slate-900` — светлый фон, компоненты сообщений не видны
   - Стало: `bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 text-white`

2. **StreamingMessage — нечитаемый Markdown** ✅ исправлено:
   - Было: `prose` (светлая типографика) на тёмном фоне → текст невидим
   - Стало: `prose-invert prose` + тёмный фон баббла

3. **HomeView — chat создавался, но навигация не происходила** ✅ исправлено:
   - Бэкенд изменил поле ответа `POST /v1/chats` с `technical_id` на `chat_id`
   - Код проверял только `response.data.technical_id` → всегда `undefined` → пользователь навсегда оставался на temp-ID
   - Исправлено: проверка `technical_id || chat_id`

### Что осталось неизменным (бэкенд-интеграция)

- Все API-эндпоинты и логика стриминга (`streamingService.ts`)
- Стор авторизации (`auth.ts`) и стор ассистента (`assistant.ts`)
- Структура роутов (сами пути `/home`, `/chat/:id`, `/environments` и т.д.)
- Логика чата (streaming, retry, error handling)
