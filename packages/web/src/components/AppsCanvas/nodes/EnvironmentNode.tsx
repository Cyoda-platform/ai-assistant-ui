...
import { Globe, Link, CheckCircle, XCircle, Code, ChevronDown, ChevronUp, ArrowRight } from 'lucide-react';
...
export const EnvironmentNode: React.FC<EnvironmentNodeProps> = ({ data }) => {
  ...
  const handleSendToChat = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (onSendToChat) {
      onSendToChat(metadata, 'environment');
    }
  };

  return (
    <>
      <div className="bg-gradient-to-br from-green-600 to-green-800 rounded-lg shadow-lg border-2 border-green-400 w-[300px] relative">
        {/* Header */}
        <div className="px-3 py-2 border-b border-green-400/30 bg-green-900/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 flex-1 min-w-0">
              <Globe className="text-green-200 flex-shrink-0" size={20} />
              ...
              {isActive ? (
                ...
                <XCircle className="text-red-300 flex-shrink-0" size={16} />
              )}
            </div>
            <div className="flex items-center space-x-1 flex-shrink-0">
              ...
              >
                {isExpanded ? (
                  <ChevronUp size={16} className="text-green-200" />
                ) : (
              ...
              {onSendToChat && (
                <button
                  ...
                  className="p-1 hover:bg-green-700 rounded transition-colors"
                  title="Send environment to chat"
                >
      ...
      {isExpanded && (
        <div className="px-3 py-2 space-y-2">
          ...
          <div className="flex items-start space-x-2">
            <Link className="text-green-300 mt-0.5 flex-shrink-0" size={14} />
