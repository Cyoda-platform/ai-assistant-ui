...
export const AppNode: React.FC<{ data: AppNodeData }> = ({ data }) => {
  ...
  const getStatusColor = () => {
    switch (data.status) {
      case 'running':
        return 'from-teal-600 via-teal-500 to-teal-600 border-teal-400';
      case 'stopped':
        return 'from-gray-600 via-gray-500 to-gray-600 border-gray-400';
      case 'deploying':
        return 'from-yellow-600 via-yellow-500 to-yellow-600 border-yellow-400';
    }
  };
  ...
  return (
    <div
      ...
      className={`
        min-w-[220px] rounded-xl border-2 shadow-2xl
        bg-gradient-to-br ${getStatusColor()}
        backdrop-blur-sm transition-all duration-300
        hover:scale-105 hover:shadow-2xl hover:shadow-teal-500/20 cursor-pointer
