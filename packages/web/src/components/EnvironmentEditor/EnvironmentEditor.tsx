...
export const EnvironmentEditor: React.FC<EnvironmentEditorProps> = ({ appId, environmentId, onSendToChat }) => {
  ...
  const [showDiagrams, setShowDiagrams] = useState(true); // Show charts by default
  ...
  const environmentCharts = useMemo(() => {
    if (!environment) {
      console.log('[EnvironmentEditor] No environment loaded yet');
      return null;
    }

    console.log('[EnvironmentEditor] Environment loaded:', environment);

    // Map environment names to chart types
    // Support both exact matches and partial matches (e.g., "new-environment-1" -> "development")
    const envNameLower = environment.name.toLowerCase();
    ...
    if (exactMatches[envNameLower]) {
      envType = exactMatches[envNameLower];
    } else {
      // Try partial match - check if environment name contains any of the keywords
      if (envNameLower.includes('prod')) {
        envType = 'production';
      } else if (envNameLower.includes('stag')) {
        envType = 'staging';
      } else if (envNameLower.includes('dev') || envNameLower.includes('development')) {
        envType = 'development';
      } else if (envNameLower.includes('test')) {
        envType = 'test';
      } else {
        // Default to development for any other environment
        envType = 'development';
      }
    }

    console.log('[EnvironmentEditor] Environment name:', environment.name, '-> type:', envType);

    const envConfig = mockDiagramsConfig.environments.find(
      e => e.environmentId === envType
    );

    console.log('[EnvironmentEditor] Environment config found:', envConfig);
    console.log('[EnvironmentEditor] Charts:', envConfig?.charts);
  ...
  console.log('[EnvironmentEditor] Render state:', {
    hasEnvironmentCharts: !!environmentCharts,
    chartsCount: environmentCharts?.length,
    editMode,
    showDiagrams,
