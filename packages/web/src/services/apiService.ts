...
class ApiService {
  ...
  async getRequirementDetail(appId: string): Promise<any> {
    return this.request(`/apps/${appId}/requirements`);
  }
  ...
  async saveRequirementDetail(appId: string, data: any): Promise<any> {
    return this.request(`/apps/${appId}/requirements`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
